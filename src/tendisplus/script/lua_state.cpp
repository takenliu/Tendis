...
int LuaState::redis_math_random(lua_State* L) {
  ...
  switch (lua_gettop(L)) {  /* check number of arguments */
    ...
    case 2: { /* lower and upper limits */
      ...
      lua_pushnumber(L,
                     floor(r * (u - l + 1)) + l); /* int between `l' and `u' */
      break;
    }
    default:
...
int LuaState::luaRedisGenericCommand(lua_State* lua, int raise_error) {
  ...
  if ((command->second->getFlags() & CMD_SORT_FOR_SCRIPT) &&
      (ls->lua_replicate_commands == 0) && expect.value().size() > 1 &&
      expect.value()[0] == '*' && expect.value()[1] != '-') {
    luaSortArray(lua);
  }
...
lua_State* LuaState::initLua(int setup) {
   ...
   * deterministic commands, when containing 'false' elements. */
  {
    std::string compare_func =
      ...
      "end\n";
    luaL_loadbuffer(
      lua, compare_func.c_str(), compare_func.length(), "@cmp_func_def");
    lua_pcall(lua, 0, 0, 0);
  }
   ...
   * of view of the user debugging a script. */
  {
    std::string errh_func =
      ...
      "end\n";
    luaL_loadbuffer(
      lua, errh_func.c_str(), errh_func.length(), "@err_handler_def");
    lua_pcall(lua, 0, 0, 0);
  }
...
void LuaState::luaMaskCountHook(lua_State* lua, lua_Debug* ar) {
  ...
  if (elapsed >= ls->_svr->getParams()->luaTimeLimit && ls->lua_timedout == 0) {
    serverLog(LL_WARNING,
              "Lua slow script detected: still in execution "
              "after %" PRId64
              " milliseconds. You can try "
...
Expected<std::string> LuaState::evalGenericCommand(Session* sess, int evalsha) {
  _sess = sess;
  const auto& args = sess->getArgs();
  int delhook = 0;
  ...
  if (!evalsha) {
    ...
    sha1hex(funcname + 2, const_cast<char*>(args[1].c_str()), args[1].length());
  } else {
  ...
  if (delhook)
    lua_sethook(_lua, NULL, 0, 0); /* Disable hook */
  if (lua_timedout) {
    lua_timedout = 0;
