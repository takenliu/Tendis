...
/* NOTE(wayenchen) stop the tasks which have the father taskid,
 * if noStopSrc, receiver will not send stop command to src Node*/
Status MigrateManager::stopTasks(const std::string& taskid, bool noStopSrc) {
  bool find = false;
  ...
  if (find && !noStopSrc) {
    ...
    LOG(INFO) << "stop srcNode taskid:" << taskid << "succ";
  }
  ...
  for (auto it = _migrateSendTaskMap.begin();
    ...
    if (taskPtr->_state == MigrateSendState::WAIT) {
      ...
      if (_cluster->clusterNodeFailed(taskPtr->_pTask->_nodeid)) {
        taskPtr->setState(MigrateSendState::ERR);
      ...
      if (taskPtr->_sender->checkSlotsBlongDst()) {
        ...
        taskPtr->setState(MigrateSendState::CLEAR);
      } else {
...
void MigrateSendTask::sendSlots() {
  ...
  if (!s.ok()) {
    if (_sender->needToWaitMetaChanged()) {
      ...
      setState(MigrateSendState::HALF);
      ...
      setState(MigrateSendState::WAIT);
      _sender->stop();
      nextSched = SCLOCK::now() + std::chrono::milliseconds(100);
    } else {
      setState(MigrateSendState::ERR);
    ...
    nextSched = SCLOCK::now();
    setState(MigrateSendState::CLEAR);
  }
...
void MigrateSendTask::deleteSenderChunks() {
  ...
  if (!_sender->checkSlotsBlongDst()) {
    ...
    if (!s.ok()) {
      ...
      setState(MigrateSendState::SUCC);
    }
  }
...
void MigrateSendTask::setState(MigrateSendState newState) {
...
uint64_t MigrateManager::getTaskNum(const std::string& pTaskid,
  ...
  if (noWait && noRunning) {
    LOG(ERROR) << "can not both forbidden wait and running task";
    return 0;
  }
...
void MigrateManager::dstPrepareMigrate(asio::ip::tcp::socket sock,
  ...
  LOG(INFO) << "sender task size is :" << taskSize;
  writer.Key("taskinfo");
  writer.StartArray();
...
void MigrateReceiveTask::checkMigrateStatus() {
  ...
  if (delay > 20 * MigrateManager::SEND_RETRY_CNT || !_receiver->isRunning()) {
    ...
    setState(MigrateReceiveState::ERR);
  }
  return;
}
...
void MigrateReceiveTask::fullReceive() {
  ...
  if (client == nullptr) {
    srcNodeFail = _svr->getClusterMgr()->getClusterState()->clusterNodeFailed(
      _pTask->_nodeid);
    if (!srcNodeFail && getRetryCount() <= retryCnt && _receiver->isRunning()) {
      ...
      LOG(ERROR) << "fullReceive with: " << _srcIp << ":" << _srcPort
                 << "taskid:" << _taskid << " failed, no valid client";
      _nextSchedTime = SCLOCK::now();
      setState(MigrateReceiveState::ERR);
    }
    _isRunning = false;
...

  {
    ...
    SCLOCK::time_point nextSched = SCLOCK::now();
    setState(MigrateReceiveState::RECEIVE_BINLOG);
    _nextSchedTime = nextSched;
...
void MigrateReceiveTask::setState(MigrateReceiveState newState) {
  _state.store(newState, std::memory_order_relaxed);
}
...
Status MigrateManager::supplyMigrateEnd(const std::string& taskid,
  ...
  SlotsBitmap slots;
  {
    ...
    if (iter != _migrateReceiveTaskMap.end()) {
      ...
      if (!binlogDone) {
        iter->second->setState(MigrateReceiveState::ERR);
        return {ErrorCodes::ERR_OK, ""};
      }
      iter->second->_receiver->setBinlogEndTime(msSinceEpoch());
      iter->second->setState(MigrateReceiveState::SUCC);
...
void MigrateManager::cleanMigrateInfo() {
  std::lock_guard<myMutex> lk(_mutex);
  _succMigrateSlots.reset();
  _failMigrateSlots.reset();
}

void MigrateManager::cleanImportInfo() {
  std::lock_guard<myMutex> lk(_mutex);
  _succImportSlots.reset();
  _failImportSlots.reset();
}
