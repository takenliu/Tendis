...
// Detailed at
// https://stackoverflow.com/questions/31474434/no-optional-in-ms-visual-studio-2013-what-to-do
...
#define my_assert(e)                                                \
  do {                                                              \
    if (!(e)) {                                                     \
      std::cout << "assert failed:" << #e << ' ' << __FILE__ << ' ' \
                << __LINE__ << std::endl;                           \
      __debugbreak();                                               \
    }                                                               \
  } while (0)
...
using OptionalBase = typename std::conditional<
  ...
  constexpr_optional_base<typename std::remove_const<T>::type>,  // use base
                                                                 // with trivial
                                                                 // destructor
...
class optional : private OptionalBase<T> {
 ...
 public:
  ...
  void swap(optional<T>& rhs) noexcept(
    is_nothrow_move_constructible<T>::value&& noexcept(
      detail_::swap_ns::adl_swap(declval<T&>(), declval<T&>()))) {
    if (initialized() == true && rhs.initialized() == false) {
...
class optional<T&> {
 ...
 public:
  ...
  constexpr typename decay<T>::type value_or(V&& v) const {
    return *this
      ? **this
      : detail_::convert<typename decay<T>::type>(constexpr_forward<V>(v));
  }
